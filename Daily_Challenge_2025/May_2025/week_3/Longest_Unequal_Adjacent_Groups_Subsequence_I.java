...
public class Longest_Unequal_Adjacent_Groups_Subsequence_I {
    ...
    public static void main(String[] args) {
        Longest_Unequal_Adjacent_Groups_Subsequence_I obj = new Longest_Unequal_Adjacent_Groups_Subsequence_I();
        String[] words = { "e", "a", "b" };
        int[] groups = { 0, 0, 1 };
        List<String> ans = obj.getLongestSubsequence(words, groups);
        System.out.println(ans); // [e, b]

        String[] words2 = { "a", "b", "c", "d" };
        int[] groups2 = { 1, 0, 1, 1 };
        ans = obj.getLongestSubsequence(words2, groups2);
        System.out.println(ans); // [a, b, c]
    }
}

/*
 * 2900. Longest Unequal Adjacent Groups Subsequence I Solved Easy Topics
 * Companies Hint You are given a string array words and a binary array groups
 ...
 * Constraints:
 * 
 * 1 <= n == words.length == groups.length <= 100 1 <= words[i].length <= 10
 * groups[i] is either 0 or 1. words consists of distinct strings. words[i]
