public class Construct_String_With_Repeat_Limit {
    ...
    public static void main(String[] args) {
        Construct_String_With_Repeat_Limit solution = new Construct_String_With_Repeat_Limit();
        String s = "cczazcc";
        int repeatLimit = 3;
...
/*
 * 2182. Construct String With Repeat Limit Solved Medium Topics Companies Hint
 ...
 * Output: "zzcccac" 
 * Explanation: We use
 * all of the characters from s to construct the repeatLimitedString "zzcccac".
 * The letter 'a' appears at most 1 time in a row. The letter 'c' appears at
 * most 3 times in a row. The letter 'z' appears at most 2 times in a row.
 * Hence, no letter appears more than repeatLimit times in a row and the string
 * is a valid repeatLimitedString. The string is the lexicographically largest
 ...
 * times in a row, so it is not a valid repeatLimitedString. Example 2:
 * 
 * Input: s = "aababab", 
 * repeatLimit = 2 
 * Output: "bbabaa" 
 * Explanation: We use
 * only some of the characters from s to construct the repeatLimitedString
 * "bbabaa". The letter 'a' appears at most 2 times in a row. The letter 'b'
 * appears at most 2 times in a row. Hence, no letter appears more than
 * repeatLimit times in a row and the string is a valid repeatLimitedString. The
 * string is the lexicographically largest repeatLimitedString possible so we
 * return "bbabaa". Note that the string "bbabaaa" is lexicographically larger
 * but the letter 'a' appears more than 2 times in a row, so it is not a valid
 * repeatLimitedString.
 * 
 * 
 * Constraints:
 * 
 * 1 <= repeatLimit <= s.length <= 105 s consists of lowercase English letters.
 */